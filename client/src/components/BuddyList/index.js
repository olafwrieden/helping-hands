...
const BuddyList = () => {
    ...
    const updateBuddyList = (id, action) => {
        switch(action) {
            case "accept":
                setBuddies(buddies.map((item, idx) => {
                    if(item.id === id) {
                ...
                break;
            case "remove":
                console.log('this user would like to ', action, ' this buddy request or buddy')
                setBuddies(buddies.map((item, idx) => {
                    if(item.id === id) {
                ...
                buddies
                    ...
                    .map((item, idx) => {
                        ...
                        <div key={`${item.first_name} ${idx}`} className="level card is-mobile buddylist-item">
                            ...
                            <span className="icon is-medium has-text-success" onClick={() => updateBuddyList(item.id, "accept")}>
                                <i className="fas fa-check-square fa-lg"></i>
                            </span>
                            <span className="icon is-medium has-text-danger" onClick={() => updateBuddyList(item.id, "remove")}>
                    ...
                    .map((item, idx) => {
                        ...
                        <div key={`${item.first_name} ${idx}`} className="level card is-mobile buddylist-item">
                            ...
                            <div className="details">
                                <span><i className="fas fa-home fa-lg"></i>{item.address}</span> <br />
                                <span><i className="fas fa-phone fa-lg"></i> {item.phone} </span>
                            </div>
                            <span className="icon is-medium has-text-danger accepted-del-btn" onClick={() => updateBuddyList(item.id, "remove")}>
                                <i className="fas fa-ban fa-lg"></i>
                            </span>
                        </div>
                        </>
